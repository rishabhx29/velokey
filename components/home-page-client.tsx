  ...
  return (
    <>
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden">
        ...
        {!isFinished && (
          <footer
            className="relative hidden shrink-0 items-end justify-center transition-all duration-300 md:flex"
            style={{ height: showKeyboard ? "auto" : 0, overflow: "visible" }}
          >
            <div
              className={cn(
                "flex w-full origin-bottom scale-[0.68] justify-center transition-all duration-300 lg:scale-[0.8] xl:scale-[0.85]",
                !showKeyboard && "pointer-events-none scale-y-0 opacity-0"
              )}
            >
              {keyboardStyle === "magic" && (
                <MagicKeyboard
              ...
              {["normal", "split", "ortho", "compact"].includes(
                keyboardStyle
              ) && (
                <Keyboard
                  theme="classic"
            ...
            <div className="fixed right-6 bottom-6 z-50 flex items-center justify-center lg:right-10">
              <button
                onClick={() => setShowKeyboard(!showKeyboard)}
                className="rounded-full border border-transparent bg-background/50 p-2 text-muted-foreground backdrop-blur-sm transition-colors hover:border-border hover:bg-muted hover:text-foreground"
                title={showKeyboard ? "Hide keyboard" : "Show keyboard"}
              >
                {showKeyboard ? (
                  <IconKeyboardHide size={20} />
                ) : (
                  <IconKeyboardShow size={20} />
                )}
              </button>
            </div>
          </footer>
