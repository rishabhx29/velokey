    ...
    <div>
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 10px 30px rgba(0,0,0,0.65), inset 0 1px 0 rgba(255,255,255,0.06), inset 0 -1px 0 rgba(0,0,0,0.3)",
        }}
      >
          ...
          <div className="space-y-[5px]">
            ...
            <Row>
              ...
              {[1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map((n) => (
                <DK key={n} kc={`Digit${n}` as KEYCODE} l={lbl(`Digit${n}`)} />
              ))}
              <DK kc={KEYCODE.Minus} l={lbl("Minus")} />
              <DK kc={KEYCODE.Equal} l={lbl("Equal")} />
              <K kc={KEYCODE.Backspace} w={100}>
            ...
            {/* QWERTY Row */}
            <Row>
              <K kc={KEYCODE.Tab} w={75}>
                tab
              </K>
              {["Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P"].map((c) => (
                <DK key={c} kc={`Key${c}` as KEYCODE} l={lbl(`Key${c}`)} />
              ))}
            ...
            <Row>
              ...
              <K kc={KEYCODE.ShiftRight} w={77}>
                shift
              </K>
              <K kc={KEYCODE.ArrowUp}>
            ...
            {/* Modifier Row */}
            <Row>
              <K kc={KEYCODE.ControlLeft} w={62}>
                ctrl
              </K>
              <K kc={KEYCODE.AltLeft} w={62}>
                option
              </K>
              <K kc={KEYCODE.MetaLeft} w={62}>
                <IconCommand className="size-[12px]" />
              </K>
              <K kc={KEYCODE.Space} w={314} />
              <K kc={KEYCODE.MetaRight}>
                <IconCommand className="size-[12px]" />
              </K>
              <K kc={KEYCODE.Fn}>fn</K>
              <K kc={KEYCODE.ControlRight}>ctrl</K>
              <K kc={KEYCODE.ArrowLeft}>
                <IconChevronLeft className="size-[12px]" />
              </K>
              <K kc={KEYCODE.ArrowDown}>
                <IconChevronDown className="size-[12px]" />
              </K>
              <K kc={KEYCODE.ArrowRight}>
...
function K({ w = 50, children, className, kc }: MechKeyProps) {
  ...
  return (
    <button
      ...
      className="flex cursor-pointer touch-none appearance-none items-end justify-center border-0 bg-transparent p-0 focus:outline-none"
    >
      <div
        className="relative flex items-center justify-center rounded-[5px]"
        style={{
          width: `${innerW}px`,
      ...
      >
        ...
        <div
          className="pointer-events-none absolute inset-0 rounded-[5px] opacity-[0.03]"
          style={{
            backgroundImage:
