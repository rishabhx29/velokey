...
function resolveSoundDef(
  ...
  const builtin =
    phase === "down"
      ? MAGIC_SOUND_DEFINES_DOWN[keyCode]
      : MAGIC_SOUND_DEFINES_UP[keyCode]
  ...
  useEffect(() => {
    ...
    const initAudio = async () => {
      ...
      try {
        ...
        const fetchRawBuffer = fetchRawAudio(soundUrl)
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={cn(
          "magic-keyboard-root mx-auto w-fit [zoom:0.75] [-webkit-text-size-adjust:100%] [text-size-adjust:100%] sm:[zoom:1] md:[zoom:1.2] lg:[zoom:1.4] xl:[zoom:1.8]",
          className
        )}
      >
  ...
  return (
    <div className="h-full w-fit rounded-[10px] bg-neutral-200 p-1 shadow-sm ring-1 shadow-black/5 ring-black/5 dark:bg-neutral-800 dark:ring-white/10">
      ...
      <Row>
        ...
        <Key
          keyCode="F13"
          containerClassName="rounded-tr-[10px]"
...
const DualKey = ({ keyCode, width }: { keyCode: string; width?: number }) => {
  ...
  if (!labels)
    return <Key keyCode={keyCode} className={width ? `w-[${width}px]` : ""} />
...
const Key = ({
  ...
  const { onMouseDown, onMouseUp, onMouseLeave } = useKeyPointerHandlers(
    keyCode,
    isPressed
  )
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      <button
        ...
        data-no-click-sound
        className={cn(
          "flex h-6 w-6 cursor-pointer items-center justify-center rounded-[3.5px] bg-gray-100 transition-all duration-75 focus:outline-none active:scale-[0.98] dark:bg-neutral-900",
...
const useKeyPointerHandlers = (
  keyCode: string | undefined,
  isPressed: boolean
) => {
  const { pressKey, releaseKey, triggerPointerHaptic } = useKeyboard()
  ...
  return { onMouseDown, onMouseUp: release, onMouseLeave: release }
}
