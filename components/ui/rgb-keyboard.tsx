...
function RGBKeyboardKeys() {
  ...
  return (
    <div ref={wrapperRef} style={{ "--rgb-hue": "0" } as React.CSSProperties}>
      ...
      <div
        className="h-fit w-fit rounded-[16px] p-4"
        style={{
          background: "linear-gradient(160deg, #16161a, #1a1a20, #141418)",
          border: "3px solid #08080a",
          boxShadow:
            "0 10px 30px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.08), inset 0 -1px 0 rgba(0,0,0,0.4)",
        }}
      >
        {/* Exposed plate/PCB between keys */}
        <div
          className="h-[290px] overflow-hidden rounded-[8px]"
          style={{
            background: "#08080a",
        ...
        >
          <div className="space-y-[5px]">
            ...
            <Row>
              ...
              <K kc={KEYCODE.F10} g={g()}>
                <IconVolume3 className="size-[10px]" />
                <span>F10</span>
              </K>
              <K kc={KEYCODE.F11} g={g()}>
                <IconVolume2 className="size-[10px]" />
                <span>F11</span>
              </K>
              <K kc={KEYCODE.F12} g={g()}>
                <IconVolume className="size-[10px]" />
                <span>F12</span>
              </K>
              <K kc={KEYCODE.F13} g={g()}>
                <IconFrame className="size-[10px]" />
              </K>
              <K kc={KEYCODE.Delete} g={g()}>
                del
              </K>
              <K kc={KEYCODE.F14} g={g()}>
            ...
            <Row>
              ...
              <K kc={KEYCODE.Backspace} w={100} g={g()}>
                <IconArrowNarrowLeft className="size-[12px]" />
              </K>
              <K kc={KEYCODE.PageUp} g={g()}>
                pgup
              </K>
            </Row>
            ...
            {/* Home Row */}
            <Row>
              <K kc={KEYCODE.CapsLock} w={100} g={g()}>
                caps lock
              </K>
              ...
              <K kc={KEYCODE.Enter} w={100} g={g()}>
                return
              </K>
              <K kc={KEYCODE.Home} g={g()}>
                home
              </K>
            </Row>
            ...
            {/* Bottom Letter Row */}
            <Row>
              <K kc={KEYCODE.ShiftLeft} w={123} g={g()}>
                shift
              </K>
              {["Z", "X", "C", "V", "B", "N", "M"].map((c) => (
                <DK
              ...
              <K kc={KEYCODE.ShiftRight} w={77} g={g()}>
                shift
              </K>
              <K kc={KEYCODE.ArrowUp} g={g()}>
                <IconChevronUp className="size-[12px]" />
              </K>
              <K kc={KEYCODE.End} g={g()}>
                end
              </K>
            </Row>
            ...
            {/* Modifier Row */}
            <Row>
              <K kc={KEYCODE.ControlLeft} w={62} g={g()}>
                ctrl
              </K>
              <K kc={KEYCODE.AltLeft} w={62} g={g()}>
                option
              </K>
              <K kc={KEYCODE.MetaLeft} w={62} g={g()}>
                <IconCommand className="size-[12px]" />
              </K>
              <K kc={KEYCODE.Space} w={314} g={g()} />
              <K kc={KEYCODE.MetaRight} g={g()}>
                <IconCommand className="size-[12px]" />
              </K>
              <K kc={KEYCODE.Fn} g={g()}>
                fn
              </K>
              <K kc={KEYCODE.ControlRight} g={g()}>
                ctrl
              </K>
              <K kc={KEYCODE.ArrowLeft} g={g()}>
                <IconChevronLeft className="size-[12px]" />
              </K>
              <K kc={KEYCODE.ArrowDown} g={g()}>
                <IconChevronDown className="size-[12px]" />
              </K>
              <K kc={KEYCODE.ArrowRight} g={g()}>
...
function K({
  ...
  const { pressedKeys, pressKey, releaseKey, triggerPointerHaptic } =
    useKeyboardContext()
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative flex h-[50px] items-start justify-center rounded-[5px] transition-all duration-75",
          active && "h-[46px]"
        )}
        style={{
          ...shellStyle,
      ...
      >
        ...
        <div
          className="pointer-events-none absolute top-[2px] bottom-[6px] left-0 z-20 w-px rounded-full"
          style={{
            background: active
        ...
        <div
          className="pointer-events-none absolute top-[2px] right-0 bottom-[6px] z-20 w-px rounded-full"
          style={{
            background:
        ...
        {!active && (
          <div
            className="pointer-events-none absolute right-[1px] bottom-0 left-[1px] z-10 h-[5px] rounded-b-[4px]"
            style={{
        ...
        <div
          className={cn(
            "absolute right-0 bottom-0 z-0 h-[2px] w-8 translate-x-3.5 rotate-70 transition-all duration-75",
            active && "rotate-60"
          )}
          style={{
            background: active
        ...
        <div
          className={cn(
            "absolute bottom-0 left-0 z-0 h-[2px] w-8 -translate-x-3.5 -rotate-70 transition-all duration-75",
            active && "-rotate-60"
          )}
          style={{
            background: active
