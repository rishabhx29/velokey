// ── Result Validation (runtime-neutral) ──────────────────────────────────────
// Anti-cheat heuristics shared by the web client (lib/validate-result.ts wraps
// this for ResultStats) and the PartyKit race server (rejects spoofed finishes
// before they reach the leaderboard). Must not import DOM/Node-only modules.

// ─── Input ───────────────────────────────────────────────────────────────────

export interface WpmSnapshotLike {
  second: number
  wpm: number
  raw: number
  errors: number
...
/** Minimal stat shape required for validation — both client and server satisfy it. */
...
function isFlatHistory(history: WpmSnapshotLike[], wpm: number): boolean {
...
function hasPerfectConsistency(
  consistency: number,
  wpm: number,
  historyLength: number
): boolean {
  if (historyLength < MIN_HISTORY_FOR_STATS || wpm < MIN_WPM_FOR_BOT_CHECKS)
    return false
  // consistency = 100 − (σ/μ × 100). ≥ 99 means σ/μ ≤ 0.01 — inhuman steadiness.
  return consistency >= 99
}

function hasImpossibleBurst(history: WpmSnapshotLike[]): boolean {
  return history.some((s) => s.wpm > MAX_BURST_WPM)
}

function detectAfk(history: WpmSnapshotLike[]): boolean {
 ...
 * `consistency` is optional (defaults to 0 = never triggers the bot check)
 * for callers that don't compute it.
 */
export function validateResultStats(
  stats: ValidatableResult,
  consistency = 0
): ValidationResult {
  const {
    wpm,
