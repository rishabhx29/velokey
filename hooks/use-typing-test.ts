...
import { generateWords, generateWordsFromPool, generateFocusWords, type Difficulty } from "@/lib/words";
...
interface UseTypingTestProps {
  ...
  onProgressUpdate?: (progress: { wordIndex: number; wpm: number; accuracy: number; totalWords: number }) => void;
...
export function useTypingTest({
  ...
  const wpm = started && !finished
    ? (wpmHistory[wpmHistory.length - 1]?.wpm ?? 0)
    : 0;
  ...
  }, [typed, wordIndex, accuracy, words.length, started, finished, startTime, onProgressUpdate]);
  ...
  }, [buildResultStats, onFinished, onTypingActiveChange, onRaceFinish]);
  ...
  const resetTestWith = useCallback(async (overrides: ResetOverrides = {}) => {
    ...
    const wc = m === "time" ? 200 : (m === "words" || m === "brainrot" || m === "focus") ? wo : 100;
    ...
    if (raceWords && raceWords.length > 0) {
      ...
      const targetKeys = worst.length > 0 ? worst : ["e", "t", "a", "o", "i", "n", "s", "r"];
      const baseWords = await buildWords(lang, wc * 3, { punctuation: p, numbers: n, difficulty: d, showDiacritics: sd });
  ...
  }, [mode, quoteLength, wordOption, timeOption, punctuation, numbers, difficulty, language, showDiacritics, customText, customCodeLanguage, codeLanguage, codeChapter, buildWords, onFinished, onTypingActiveChange]);
  ...
  useMountEffect(() => {
    ...
    if (m === "quote") {
      ...
      const worst = getWorstKeys(6);
      const targetKeys = worst.length > 0 ? worst : ["e", "t", "a", "o", "i", "n", "s", "r"];
      buildWords(language, wc * 3, { punctuation: false, numbers: false, difficulty }).then((baseWords) => {
  ...
  useEffect(() => {
    if (prevLangRef.current !== language) {
      ...
      queueMicrotask(() => { void resetTestWith({ language }); });
    }
  ...
  useEffect(() => {
    if (prevShowDiacriticsRef.current !== showDiacritics) {
      ...
      queueMicrotask(() => { void resetTestWith({ showDiacritics }); });
    }
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      requestAnimationFrame(() => measurePerformance("typing-keydown-to-frame", "typing-keydown"))
    ...
    [
      ...
      recordWordSnapshot, markTypingActive, onTypingActiveChange, onWrongKey, clearWordOrNavigateBack, buildResultStats,
    ],
  );
